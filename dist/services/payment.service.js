 ...
 */
const PAYABLE_BOOKING_STATUSES = ['accepted'];
function computeAmount(booking, type) {
    switch (type) {
        case 'rental':
...
exports.paymentService = {
    async initialize(userId, input) {
        const booking = await bookingRepository.findById(input.bookingId);
        if (!booking)
            throw AppError_1.AppError.notFound('Booking not found');
        if (booking.renterId !== userId) {
            throw AppError_1.AppError.forbidden('You can only pay for your own bookings');
        }
        if (!PAYABLE_BOOKING_STATUSES.includes(booking.status)) {
            throw AppError_1.AppError.badRequest(`This booking is "${booking.status}" and cannot be paid for right now`);
        }
        const amount = computeAmount(booking, input.type);
        if (!amount || amount <= 0) {
            throw AppError_1.AppError.badRequest('Nothing to charge for this payment type');
        }
        ...
        const paystackData = await paystack_service_1.paystackService.initializeTransaction({
            email: user.email,
    ...
    async handleWebhook(rawBody, signature) {
        if (!paystack_service_1.paystackService.verifyWebhookSignature(rawBody, signature)) {
            throw AppError_1.AppError.unauthorized('Invalid webhook signature');
        }
        ...
        if (!payment)
            return; // unknown reference — nothing to reconcile
        if (payment.status === 'successful')
            return; // already processed, avoid double-effects
    ...
    async getById(userId, paymentId) {
        ...
        if (!payment)
            throw AppError_1.AppError.notFound('Payment not found');
        if (payment.userId !== userId)
            throw AppError_1.AppError.forbidden('You cannot view this payment');
        return payment;
    },
    async myPayments(userId, page, limit) {
     ...
     */
    async requestRefund(userId, paymentId, reason) {
        const payment = await paymentRepository.findById(paymentId);
        if (!payment)
            throw AppError_1.AppError.notFound('Payment not found');
        if (payment.userId !== userId) {
            throw AppError_1.AppError.forbidden('You cannot request a refund for this payment');
        }
        if (payment.status !== 'successful') {
            throw AppError_1.AppError.badRequest('Only successful payments can be refunded');
        }
        const existing = await refundRepository.findOne({ paymentId: payment.id, status: 'pending' });
        if (existing) {
            throw AppError_1.AppError.conflict('A refund request for this payment is already pending');
        }
